...
class GeminiService {
  ...
  private findClosestAIKeyword(text: string): string | null {
    const lowerText = text.toLowerCase();
    let bestMatch: string | null = null;
    let bestScore = 0;

    for (const keyword of this.aiKeywords) {
      const score = stringSimilarity(lowerText, keyword.toLowerCase());
      if (score > bestScore) {
        bestScore = score;
        bestMatch = keyword;
      }
    ...
    // Threshold: if similarity is above 0.75, assume it's an intended AI term
...
1. You can ONLY discuss AI topics including: machine learning, deep learning, neural networks, NLP, computer vision, reinforcement learning, generative AI, AI tools, AI history, AI applications, AI ethics, AI careers, and related fields.
2. If asked about ANYTHING outside AI, you MUST respond with: "I can only help with AI-related topics. Please ask me something related to AI."
3. Correct common spelling mistakes (e.g., "dip learning" → "deep learning") when they clearly refer to an AI topic.
4. Keep responses clear, accurate, and helpful.
5. Maintain a professional but friendly tone.
6. If unsure whether a topic is AI-related, decline by default.`;
  }
  ...
  async generateResponse(userMessage: string): Promise<string> {
    ...
    // Try to correct spelling/misspellings
    const closestKeyword = this.findClosestAIKeyword(userMessage);
...
User question: ${userMessage}
Detected AI topic: ${closestKeyword}
